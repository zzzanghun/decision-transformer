...
from decision_transformer.training.seq_trainer_toy import SequenceTrainer
from auto_encoder.model import SparseVoxelAutoencoder   
...
def make_straight_episode(episode_num: int):
    """
    0 → 1 → 2 … → episode_num 으로 직진하는 단일 에피소드를 생성한다.
    - 행동 1: +1 칸 전진
    - 행동 0: (터미널 스텝) 더 이상 행동 없음

    반환값
    -------
    episode_data : dict
        'observations': [0, 1, …, episode_num]
        'actions'    : [1, 1, …, 1, 0]      # 마지막은 dummy
        'rewards'    : [1, 1, …, 1]          # 모든 스텝 +1
        'terminals'  : [F, F, …, F, T]       # 마지막만 True
    """
    observations = np.arange(episode_num + 1, dtype=np.int32)

    # 마지막 스텝은 이미 종료 상태이므로 의미 없는 dummy(0) 액션을 넣어 둡니다.
    actions = np.array(
        [1 if t < episode_num else 0          # 0: dummy, 1: 오른쪽 전진
         for t in range(episode_num + 1)],
        dtype=np.int32
    )

    rewards = np.zeros(episode_num + 1, dtype=np.float32)
    rewards[-1] = 1.0
    terminals = np.array(
        [False] * episode_num + [True],       # 마지막만 True
        dtype=bool
    )
...
def experiment(
    ...
    if env_name == 'toy':
        trajectories = []
        for episode_num in range(100, 1000):
            episode_data = make_straight_episode(episode_num)
    ...
    def get_batch(batch_size=10, max_len=K):
    ...
    for iter in range(variant['max_iters']):
        ...
        if (iter + 1) % 1000 == 0:
            ...
            folder_name = f"{PROJECT_PATH}/model/fm/{iter + 1}_{min_action_error:e}"
